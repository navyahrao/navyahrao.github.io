...
const Header = () => {
  ...
  return (
    ...
    >
      <Link to='/' onClick={() => handleLinkClick(0)} className='py-2.5 mx-2.5 text-3xl font-sans font-bold text-violet-500'>
        <div>Navya Rao</div>
      </Link>
          ...
          {links.map(({ name, url }, index) => (
            <Link
              ...
              className={`flex max-h-min px-2 py-2.5 md:px-4 rounded-md bg-white font-medium mx-2.5 transition-all duration-300 ease-in-out hover:text-white hover:bg-gradient-to-r from-pink-200 to-violet-400 items-center ${
                currentIndex === index ? 'bg-gradient-to-r text-white from-pink-300 to-violet-500' : ''
              }`}
            >
              <div>{name}</div>
            </Link>
      ...
      >
        <span className={`block w-6 h-1 bg-violet-500 mb-1 ${isMobileMenuOpen ? 'absolute top-0 transform rotate-45' : ''} `}></span>
        <span className={`block w-6 h-1 bg-violet-500 mb-1 ${isMobileMenuOpen ? 'opacity-0' : 'opacity-100'} `}></span>
        <span className={`block w-6 h-1 bg-violet-500 mb-1 ${isMobileMenuOpen ? 'absolute top-0 transform -rotate-45' : ''} `}></span>
      </button>
